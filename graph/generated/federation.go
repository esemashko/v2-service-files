...
func (ec *executionContext) resolveEntity(
	...
	case "File":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "File": %w`, err)
		}
		switch resolverName {
		...
		case "findFileByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findFileByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindFileByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "File": %w`, err)
			}
...
func entityResolverNameForFile(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for File", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for File", ErrTypeNotFound))
			break
		}
		return "findFileByID", nil
	}
	return "", fmt.Errorf("%w for File due to %v", ErrTypeNotFound,
